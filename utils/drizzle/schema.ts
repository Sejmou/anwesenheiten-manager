...
export const event = pgTable("Event", {
	...
	lastSyncAt: timestamp("lastSyncAt", { precision: 3, mode: 'string' }).notNull(),
...
export const setlistSongInfo = pgTable("SetlistSongInfo", {
	...
	setlistId: text("setlist_id").notNull().references(() => setlist.id, { onDelete: "cascade" } ),
	songId: text("song_id").notNull().references(() => song.id, { onDelete: "cascade" } ),
	order: integer("order").notNull(),
	notes: text("notes"),
},
(table) => {
	return {
		setlistIdSongIdOrderKey: uniqueIndex("SetlistSongInfo_setlist_id_song_id_order_key").on(table.setlistId, table.songId, table.order),
	}
});
