...
const Members: NextPageWithLayout = () => {
  const { addMessage } = useStoreActions(actions => actions);
  ...
  const handleFiles = async (files: File[]) => {
    ...
    if (importedSingers.length === 0) {
      alert(
        'Keine gültigen Daten gefunden!\nDie CSV-Datei muss mit einer Zeile für die Spaltenüberschriften beginnen ("Vorname", "Nachname" und "Stimmgruppe", in beliebiger Reihenfolge).'
      );
      return;
    }
  ...
  const singersMutation = useMutation(addSingers, {
    onSuccess: () => {
      ...
      addMessage('Mitglieder erfolgreich importiert!');
    },
  });
